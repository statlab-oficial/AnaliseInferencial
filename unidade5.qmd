...
z_alpha = norm.ppf(1 - alpha/2) 
z_beta = norm.ppf(power) 
n0 = (p*(1-p))*((z_alpha + z_beta) * 1 / delta)**2 
...
z_alpha = norm.ppf(1 - alpha/2) 
z_beta = norm.ppf(power) 
n0 = (proporcao_catolica*(1-proporcao_catolica))*((z_alpha + z_beta) * 1 / delta)**2 
n = (n0 * N)/(n0 + N - 1) 
n = int(np.ceil(n)) 

print(f"Tamanho da amostra: {np.ceil(n):.0f}")

```

### Seleção da amostra aleatória

Agora iremos selecionar aleatoriamente os 2920 estudantes.
...
Além disso, a estatística t segue uma distribuição t-Student, e os graus de liberdade (gl) são calculados pela fórmula de *Welch-Satterthwaite*:
...
Ao utilizar tabelas da distribuição t, recomenda-se arredondar os graus de liberdade para baixo até o inteiro mais próximo. 
...
e o tamanho da amostra do segundo grupo é $n_2 = k n_1$, em que $q_1 = 1-p_1$ e
$q_2 = 1-p_2$. Além disso,

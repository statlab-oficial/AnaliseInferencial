...
$$\mathbf{x}=(x_1,x_2,\ldots,x_n).$$ Esta distinção é o pilar da inferência estatística: enquanto a amostra observada $\mathbf{x}$ é um conjunto de números fixos, a amostra aleatória $\mathbf{X}$ é um vetor de variáveis aleatórias.
...
2.  Estimador: é uma estatística utilizada especificamente com o objetivo de aproximar um parâmetro populacional desconhecido ($\theta$). Como o estimador é uma função de variáveis aleatórias, ele próprio é uma variável aleatória. Portanto, cada estimador possui sua própria distribuição amostral, que descreve como seus valores variariam se extraíssemos infinitas amostras da mesma população.
...
## Estatística como função da amostra
...
### Exemplos Fundamentais {.unnumbered}

1.  Média Amostral ($\bar{X}$): É a soma das observações dividida pelo tamanho da amostra, ou seja

$$\bar{X} = \frac{1}{n}\sum\limits_{i=1}^{n} X_i.$$ Sendo o estimador natural da média populacional $\mu$.
...
$$S^2 = \frac{1}{n-1}\sum\limits_{i=1}^{n}(X_i - \bar X)^2.$$ 3. Proporção Amostral ($\hat p$): Aplicada quando a população segue uma distribuição de Bernoulli, onde cada unidade é classificada binariamente (sucesso/fracasso), ou seja,

$$\hat p = \frac{1}{n}\sum\limits_{i=1}^{n} X_i,$$ em que $X_i$ assume o valor 1 para "sucesso" e 0 para "fracasso".

## O Experimento da Urna e Variáveis de Bernoulli
...
a estatística $R = \sum\limits_{i=1}^{n} X_i$ representa o número total de sucessos e segue uma distribuição $\text{Binomial}(n,p)$. A proporção amostral $\hat p = R/n$ é, portanto, a média aritmética dessas variáveis de Bernoulli, funcionando como um estimador não-viesado para a proporção populacional $p$.
...
1.  Esperança: $\operatorname{E}(\widehat{p})=p$ (o que torna $\hat p$ um estimador não-viesado).

2.  Variância: $\operatorname{Var}(\widehat{p})=\frac{p(1-p)}{n}$.

3.  Erro-Padrão: É o desvio padrão da distribuição amostral, dado por $\operatorname{EP}(\widehat{p}) = \sqrt{\frac{p(1-p)}{n}}$.
...
::: {.callout-note title="Modelo exato para a urna"}
Quando a amostragem é realizada sem reposição em uma população finita de tamanho $N$ (com $M$ bolas vermelhas), as observações não são independentes. O número de sucessos segue, então, uma distribuição $\text{Hipergeométrica} (N,M,n)$.
...
## Critérios básicos para avaliar estimadores
...
1.  O viés de um estimador é a diferença entre o seu valor esperado e o valor real do parâmetro, sendo definido por $\operatorname{E}(\hat \theta) - \theta$. Um estimador é considerado não viesado quando, em média, seus valores coincidem com o parâmetro alvo, ou seja, $\operatorname{E}(\hat \theta) = \theta$.

2.  A variância, por sua vez, quantifica a dispersão ou incerteza do estimador entre diferentes amostras possíveis. Um critério abrangente que permite comparar estimadores viesados e não viesados é o Erro Quadrático Médio (EQM), que combina viés e variabilidade através da decomposição $\operatorname{EQM}(\hat \theta) = \operatorname{Var}(\hat \theta) + [\text{Viés}(\hat \theta)]^2$.
...
1.  Qual é a diferença entre $\widehat{p}$ e o valor observado de $\widehat{p}$?

2.  Por que um estimador é considerado uma variável aleatória?

3.  Qual é a relação entre o histograma das proporções obtidas nas amostras repetidas e a distribuição amostral de $\widehat{p}$?

4.  O que acontece com a variabilidade de $\widehat{p}$ quando o tamanho da amostra aumenta?
...
em que $\theta$ representa o parâmetro (ou vetor de parâmetros) desconhecido que rege o mecanismo teórico, $\Theta$ é o espaço paramétrico, que compreende todos os valores possíveis que $\theta$ pode assumir. Cada valor de $\theta$ especifica uma distribuição de probabilidade distinta para os dados.

> O objetivo central da inferência é utilizar a amostra observada para identificar quais valores de $\theta$ dentro do espaço paramétrico são mais compatíveis com a evidência empírica coletada.

## Definição da função de verossimilhança

Considere uma amostra observada $\mathbf{x}=(x_1,x_2,\ldots,x_n)$ proveniente de um PGD com função de probabilidade (ou densidade) $f(x;\theta)$. Sob a suposição de que as observações são i.i.d., a distribuição conjunta da amostra é dada pelo produto das densidades individuais:

$$f(\mathbf{x}; \theta) = \prod\limits_{i=1}^{n}f(x_i;\theta).$$ Quando tratamos essa expressão como uma função de $\theta$ , mantendo os dados observados $\mathbf{x}$ fixos, ela é denominada função de verossimilhança:

$$L(\theta; \mathbf{x}) = \prod\limits_{i=1}^{n}f(x_i;\theta).$$ A verossimilhança quantifica a compatibilidade relativa entre diferentes valores do parâmetro e os dados efetivamente coletados, servindo de base para métodos de estimativa e tomada de decisão.

::: {.callout-warning title="Probabilidade e verossimilhança"}
É fundamental distinguir os dois conceitos:

1.  Na probabilidade $f(\mathbf{x}; \theta)$, o parâmetro $\theta$ está fixo e variamos os dados $\mathbf{x}$ para prever possíveis resultados.

2.  Na verossimilhança $L(\theta; \mathbf{x})$ os dados $\mathbf{x}$ estão fixos e variamos $\theta$ para avaliar qual modelo melhor explica o que foi observado.

Portanto, $L(\theta; \mathbf{x})$ não é uma distribuição de probabilidade para $\theta$. Ela não indica a probabilidade de o parâmetro ser um determinado valor, mas sim o quão plausível é aquele valor dado o resultado da amostra.
:::
...
$$X_i \sim \text{Bernoulli}(p),$$ em que $X_i = 1$ indica uma bola vermelha e $X_i = 0$ uma bola branca. A função de probabilidade para uma única observação é $f(x_i;p) = p^{x_i}(1-p)^{1-x_i}$ para $0 \leq p \leq 1$. Para uma amostra de $n$ bolas, a função de verossimilhança é:

$$L(p; \mathbf{x}) = \prod\limits_{i=1}^{n}p^{x_i}(1-p)^{1-x_i}.$$ Se definirmos $r = \sum\limits_{i=1}^{n} x_i$ como o número total de bolas vermelhas, a expressão simplifica-se para:

$$L(p; r) = p^{r}(1-p)^{n-r}.$$ Este resultado demonstra que, para inferir sobre a proporção $p$, a ordem das bolas é irrelevante, apenas o total $r$ de sucessos importa.

...
Sendo o estimador natural da média populacional $\mu$.
...
Quando tratamos essa expressão como uma função de $\theta$ , mantendo os dados observados $\mathbf{x}$ fixos, ela é denominada função de verossimilhança:
...
$$L(p; \mathbf{x}) = \prod\limits_{i=1}^{n}p^{x_i}(1-p)^{1-x_i}.$$
Se definirmos $r = \sum\limits_{i=1}^{n} x_i$ como o número total de bolas vermelhas, a expressão simplifica-se para:

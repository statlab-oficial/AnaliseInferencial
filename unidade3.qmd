...
Considere $X \sim \text{Exp}(\lambda)$ com $E(X) = \frac{1}{\lambda}$ e $V(X) = \frac{1}{\lambda^2}$.
$$E_{\lambda}(X) = \frac{1}{\lambda} = \bar{X} \implies \widehat{\lambda} = T_4 = \frac{1}{\bar{X}}$$

*Nota sobre as propriedades:* Sabemos que $S = \sum X_i \sim \text{Gama}(n, \lambda)$. Pode-se demonstrar que $V = \bar{X} \sim \text{Gama}(n, n\lambda)$. Calculando a esperança da inversa:
$$E(T_4) = E\left(\frac{1}{V}\right) = \frac{n}{n-1}\lambda \neq \lambda \quad (\text{para } n > 1)$$
Logo, $T_4$ é viciado, mas **assintoticamente não viciado**. A variância, após integração, resulta em:
$$Var(T_4) = \frac{n^2}{(n-1)^2(n-2)}\lambda^2$$
Como $\lim_{n \to \infty} Var(T_4) = 0$, $T_4$ é um estimador consistente.

### Exemplo 5: Variáveis Independentes Não Identicamente Distribuídas
Seja $Y_i \sim N(\beta X_i, \sigma^2)$ para $i = 1, \dots, n$, com $X_i$ constantes conhecidas.
Como $E(Y_i) = \beta X_i$, usamos uma definição alternativa para o método dos momentos:
...
É fácil verificar que $E(\widehat{\beta}) = \beta$ e $Var(\widehat{\beta}) = \frac{\sigma^2}{n\bar{X}^2}$.

### Exemplo 6: Normal (Média Zero)
Seja $X \sim N(0, \sigma^2)$. Queremos estimar $\sigma^2$. 
Temos $E(X) = 0$ e $E(X^2) = \sigma^2$. Usando o segundo momento:
$$E(X^2) = \frac{\sum_{i=1}^n X_i^2}{n} \implies \widehat{\sigma^2} = T = \frac{\sum_{i=1}^n X_i^2}{n}$$
Com a curtose da Normal (3), temos $V(X^2) = 2\sigma^4$. Logo, $Var(T) = \frac{2\sigma^4}{n}$.
